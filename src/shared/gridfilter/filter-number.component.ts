...
@Component({
selector: '<filter-numbercomponent></filter-numbercomponent>',
providers: [CUSTOM_INPUT_CONTROL_VALUE_ACCESSOR ],
template: `<div  class="filter-block">
          
          <span style="width:80% !important" class="filter-textbox">
              <input type='number' [(ngModel)]="value" (ngModelChange)="emitValue($event)" [formControl]="filterInput" class="ui-column-filter (keyup.enter)="fnEnter($event)">
          </span>
...
ngOnInit() 
{
  this.optionitems  = [
         { label: 'Equal', icon: 'fa-plus', command: (event) => this.optionClick(event, 'equals')  },
            ...
            { label: 'Greater than', icon: 'fa-plus', command: (event) => this.optionClick(event,  'greaterthan') },
            { label: 'Less than', icon: 'fa-plus', command: (event) => this.optionClick(event,  'lessthan') },
            { label: 'Greaterthan Equal', icon: 'fa-plus', command: (event) => this.optionClick(event,  'greaterthanequal') },
            { label: 'Lessthan Equal', icon: 'fa-plus', command: (event) => this.optionClick(event,  'lessthanequal') },
